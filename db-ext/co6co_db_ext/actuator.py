...
class Actuator:
    ...
    @staticmethod
    def remove_db_instance_state(
        poInstance_or_poList: Iterator | Any,
    ) -> List[Dict] | Dict:
        if hasattr(poInstance_or_poList, "__iter__") and not isinstance(
            poInstance_or_poList, str
        ):
            result = [
                dict(
                    filter(
                        lambda k: k[0] != Actuator.__po_has_field__, a1.__dict__.items()
                    )
                )
                for a1 in poInstance_or_poList
            ...
            for r in result:
                for r1 in r:
                    value = r.get(r1)
                    if hasattr(
                        value, Actuator.__po_has_field__
        ...
        # and hasattr (poInstance_or_poList,"__dict__")
            ...
            return dict(
                filter(
                    lambda k: k[0] != Actuator.__po_has_field__,
                    poInstance_or_poList.__dict__.items(),
                )
            )
        else:
            return poInstance_or_poList
    ...
    async def query_one_mappings(
        self, select: Select, params: Dict | Tuple | List = None
    ):
        """
        select:Select
        return dict
    ...
    async def query_one_entity_mapping(
        self, select: Select, params: Dict | List | Tuple = None
    ):
    ...
    async def batchAdd(
        ...
        poList: List[BasePO],
        userId=None,
        checkFun: Callable[[POType, AsyncSession], Any] = None,
        afterFun: Callable[[List[BasePO], AsyncSession, Any], Result] = None,
    ):
        try:
            for po in poList:
...
class OperationOption:
    def __init__(self):
        ...
        self.beforeFun: TypeCheckBack = None
        self.afterFun: TypeCallBack = None
    ...
    def create_del(
        ...
        *,
        ...
        po: BasePO = None,
        poType: POType = None,
        checkFun: TypeCheckBack = None,
        afterFun: TypeCallBack = None,
    ):
        option = cls()
