...
class NATService:
    ...
    async def on_disconnected(self):
        logger.warning("NATS disconnected. Retrying in background...")
    ...
    async def publish(
        self, subject: str, message: BaseMessage, reply_to: Optional[str] = None
    ):
        """发布消息"""
        await self.connect()
    ...
    async def request(
        self, subject: str, message: BaseMessage, timeout: float = 2.0
    ) -> ResponseMessage:
        """发送请求并等待响应"""
        await self.connect()
        payload = message.to_json().encode("utf-8")
