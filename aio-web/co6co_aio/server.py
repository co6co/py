...
async def appStart(app: web.Application):
    """应用启动时的初始化操作"""
    config = app.config
    db = db_service(config["db_settings"])
    app.db = db
    pass


async def appShutdown(app: web.Application):
    """应用关闭时的清理操作"""
...
class http_server_base(metaclass=abc.ABCMeta):
    ...
    def get_config(configFile: str):
        ...
        web_config = http_server_base._get_web_config(configs)
        db_config = http_server_base._get_db_config(configs) 
        return configs,web_config,db_config

    @staticmethod
    def _get_web_config(configs:dict): 
        setting = configs.get("web_setting", {})
        web_config = webConfig()
        web_config.post_init(DictNamespace(**setting)) 
        return web_config
    @staticmethod
    def _get_db_config(configs:dict):
