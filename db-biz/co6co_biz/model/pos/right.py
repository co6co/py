...
import uuid
from model.enum.right import user_category, user_state,account_category
...
class AccountPO(UserTimeStampedModelPO):
    ...
    userId = Column("user_id", ForeignKey("{}.{}".format(UserPO.__tablename__, UserPO.id.name), ondelete="CASCADE"), nullable=True,  comment="可以先进行账号创建再绑定user_id")
    accountName = Column("account_name", String(64), unique=True)
    attachInfo = Column("attach_info", String(255), comment="有些信息需要存下来，才能配合账号使用")
    category = Column("category", INTEGER, comment=f"账号类别:{account_category.to_labels_str()}，去那张表找attachInfo")
