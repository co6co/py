        ...
        <el-scrollbar>
			...
			<el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header"> 
				...
				<el-table-column label="类型">
					<template #default="scope"> {{ from_attach_data.getTypeItem(scope.row.type)?.label }} </template>
				</el-table-column> 
				...
				<el-table-column label="操作" width="316" align="center">
					<template #default="scope">  
						 <v-download v-permiss="getPermissKey(ViewFeature.download)" :url="getDownloadUrl(scope.$index, scope.row)" :file-name="`${scope.row.name}.zip`" ></v-download>
						<!--v-permiss="16"-->
						<el-button text  v-permiss="getPermissKey(ViewFeature.del)"  :icon="Delete"  class="red" @click="onDelete(scope.$index,scope.row)" >
							删除
						</el-button> 
					</template>
				</el-table-column>
