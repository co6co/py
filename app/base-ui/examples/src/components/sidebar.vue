...
import { useRouteData, type IRouteData, type sideBarItem } from '../hook/sys/useRoute'
...
const convertSideItem = (data: IRouteData, subs?: sideBarItem[]): sideBarItem => {
  const result = {
  ...
  return { ...result, ...{ subs: subs } } as sideBarItem
}
const routeData2Item = (data: IRouteData): sideBarItem | null => {
  if (
    (data.category == RouteItemType.GROUP || data.category == RouteItemType.VIEW) &&
    data.children &&
    data.children.length > 0
  ) {
    let sub = data.children.map((d) => routeData2Item(d)).filter((m) => m)
    if (data.category == RouteItemType.GROUP && (!sub || sub.length == 0)) return null
    return convertSideItem(data, sub as sideBarItem[])
  }
  if (data.category == RouteItemType.VIEW && (!data.children || data.children.length == 0)) {
    return convertSideItem(data)
  }
  return null
}

const items = reactive<{ data: sideBarItem[] }>({ data: [] })
const getData = () => {
  nextTick(() => {
