...
import {
  ...
  useMenuState,
  MenuCateCategory
} from '../../hook/sys/useMenuSelect'
import useHttpMethods, { usePageFeature } from '../../hook/sys/useMethods'
import {
  ElRow,
  ElCol,
  ElButton,
...
import EcIconSelect from 'co6co'
  ...
  setup(prop, ctx) {
    ...
    const init_data = (oper: FormOperation, item?: Item) => {
      ...
      switch (oper) {
        case FormOperation.add:
          ...
          if (menuStateData.selectData.value && menuStateData.selectData.value.length > 0)
            DATA.fromData.status = menuStateData.selectData.value[0].value as number
        ...
        case FormOperation.edit:
          ...
          if (DATA.fromData.category == MenuCateCategory.API) {
            DATA.fromData.methods =
              item.methods && (item.methods as string) ? (item.methods as string).split(',') : []
          } else {
            //button 需要
    ...
    const rules_view: FormRules = {
      ...{
        ...
        component: [{ required: true, message: '请输入视图地址', trigger: 'blur' }],
        permissionKey: [{ required: false, message: '权限字', trigger: 'blur' }]
      },
      ...rules_b
    }
          ...
          <ElRow>
            <ElCol span={12}>
              <ElFormItem label="代码" prop="code">
                <ElInput v-model={DATA.fromData.code} placeholder="菜单代码"></ElInput>
              </ElFormItem>
            </ElCol>
            <ElCol span={12}>
              ...
              <ElRow>
                ...
                <ElCol span={12}>
                  <ElFormItem label="图标" prop="icon">
                    <EcIconSelect v-model={DATA.fromData.icon}></EcIconSelect>
                  </ElFormItem>
                </ElCol>
              </ElRow>
              <ElRow>
