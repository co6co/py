...
export default defineComponent({
  ...
  setup(prop, ctx) {
    ...
    const validUserName = (
      ...
      if (value.length < 3)
        return (rule.message = '长度应该大于3'), callback(new Error(rule.message))
      exist_svc(value, DATA.id).then((res) => {
        if (res.code == 0) return (rule.message = res.message), callback(new Error(rule.message))
        else return callback()
      })
    }
    const rules_edit: FormRules = {
      userName: [
        { required: true, message: '请输入用户名', validator: validUserName, trigger: 'blur' }
      ],
    ...
    const rules_add: FormRules = {
      ...{
        password: [
          { required: true, min: 6, max: 20, message: '请输入6-20位密码', trigger: 'blur' }
        ]
      },
      ...rules_edit
    }
      ...
      default: () => (
        <>
          <ElFormItem label="用户名" prop="userName">
            <ElInput v-model={DATA.fromData.userName} placeholder="用户名"></ElInput>
          </ElFormItem>
