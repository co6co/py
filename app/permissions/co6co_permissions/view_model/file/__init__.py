...
class FolderView(AuthMethodView):
    ...
    async def head(self ):
        ...
        """
        try:
            args = self.usable_args(self.request )
            ...
            id = self.request.headers.get("session")
            self.request.app.ctx.data = {id: {filePath: zipFilePath}}
    ...
    async def get(self ):
        try:
            args = self.usable_args(self.request )
            key = args.get("path")
            sessionId = self.request.headers.get("session")
            filePath = self.request.app.ctx.data[sessionId].get(key)
            _, end, file_size = self.parseRange(self.request, filePath=filePath)
            return await self.get_file_partial(self.request, filePath)
        finally:
            if filePath is not None and os.path.exists(filePath) and file_size-1 == end:
                # log.warn("删除文件.", filePath)
                os.remove(filePath)
                # os.unlink(zipFilePath)
                cache: dict = self.request.app.ctx.data[sessionId]
                log.warn(cache, key, cache.keys)
                if key in cache:
                    cache.pop(key)
                if {} == cache:
                    cache: dict =self. request.app.ctx.data
...
class FileViews(AuthMethodView):
    ...
    async def post(self ):
        ...
        if param.root is None:
            param.root = "/"
        if param.root.endswith(":"):
            param.root = param.root+os.sep

        def filter(x):
            return param.name is None or param.name in x
    ...
    async def put(self ):
        ...
        try:
            ...
            await self.request.stream(process_data)
