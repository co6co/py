        ...
        async def edit( one: UserPO):
            if one is not None:
                if one.password != one.encrypt(oldPassword):
                    return Result.fail(message="输入的旧密码不正确！")
                if one.encrypt(password) == one.encrypt(oldPassword):
                    return   Result.fail(message="输入的旧密码与新密码一样！") 
...
class user_avatar_view(resource_baseView):
    ...
    async def get(self):
        ...
        if avatar:
            return await self.response_local_file(avatar)
        else:
            svg_content = """
    ...
    async def put(self):
        ...
        if isinstance(result, FileResult): 
            async def edit( one: UserPO):
                if one is not None and result.path:
...
class user_info_view(AuthMethodView):
    ...
    async def get(self):
        ...
        """
        userName = self.userName
        select = Select(UserPO.avatar, UserPO.userName, UserPO.remark).filter(
