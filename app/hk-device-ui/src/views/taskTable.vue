			...
			<el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header"> 
				...
				<el-table-column label="操作" width="316" align="center">
					<template #default="scope"> 
						 ...
						 <v-download :url="getDownloadUrl(scope.$index, scope.row)" :file-name="`${scope.row.name}.zip`" ></v-download>
						<!--v-permiss="16"-->
...
import { attach_data,TaskAtachData } from "../store/tasks"; 
...
const onDownLoad=(index:number,row:any)=>{
	download_svc(import.meta.env.VITE_TASK_PATH+row.data,`${row.name}.zip`)
}
</script>
