<template>
						...
						<el-table-column label="操作" align="center">
							<template #default="scope">
								...
								<el-button
									text
									:icon="Setting"
									@click="onOpenConfigPage('router', scope.row)">
									路由器
								</el-button>
								<el-button
									text
									:icon="Setting"
									@click="onOpenConfigPage('box', scope.row)">
									AI盒子
								</el-button>
								<el-button
									text
									:icon="Setting"
									@click="onOpenConfigPage('ip_camera', scope.row)">
									监控球机
								</el-button>
							</template>
						</el-table-column>
    ...
    <!-- 弹出框 -->
		<el-dialog  title="打开配置网址" draggable class="setting" v-model="configDialog_model.showDialog" style="width:36%;" > 
      <template #header>
        <el-text>
          <el-icon>
            <Setting />
		...
		<el-dialog :title="form.title" v-model="form.dialogVisible">
			<el-form
				...
				<el-form-item label="设备代码" prop="deviceCode">
					<el-input
						v-model="form.fromData.deviceCode"
						placeholder="设备代码"></el-input>
				</el-form-item>
				<el-form-item label="设备位置" prop="postionInfo">
					<el-input
						v-model="form.fromData.postionInfo"
						type="textarea"
						placeholder="位置信息" />
				</el-form-item>
...
<style scoped lang="less">
  ...
  ::v-deep .setting{
    .el-dialog__body{padding: 10px 40px 20px 40px;}
  }
</style>
