...
import codeOption from '@/components/showCode/src/codemirrorOption';
...
export default defineComponent({
	...
	setup(prop, ctx) {
		...
		const DATA = reactive<
			FormData<number, FormItem> & { testing: boolean; testResult: string }
		>({
			...
			api
				...
				.then((res) => {
					...
					if (typeof res.data == 'object')
						DATA.testResult = JSON.stringify(res.data);
		...
		const valid = (
			promise: Promise<IResponse<boolean>>,
			rule: any,
			callback: validatorBack
		) => {
			promise.then((res) => {
		...
		const rules: FormRules = {
			...
			state: [
				{ required: true, message: '状态能为空', trigger: ['blur', 'change'] },
			],
			sourceCode: [
				{
					required: true,
					validator: validSourceCode,
					message: '源代码',
					trigger: ['blur'],
				},
			],
