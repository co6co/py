...
async def xss(request:Request): 
    ...
    async with request.ctx.session as session:  
        ...
        select=( 
            Select(bizXssPO.port,bizDevicePo.ip,bizDevicePo.name).join(bizDevicePo,isouter=True,onclause=bizDevicePo.id==bizXssPO.id)
        ) 
        result= await session.execute(select)
        server=result.fetchone() 
        await session.commit()
        return  JSON_util.response(Result.success(data=db_tools.row2dict(server), message="获取成功")) 