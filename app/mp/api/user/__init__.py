...
async def ticket(request: Request, uuid: str):
    async with request.ctx.session as session:
        ...
        log.warn(user)
        token = await generateUserToken(request, user.id, user.to_dict())
    ...
    async with request.ctx.session as session:
        ...
        if user != None:
            ...
            if user.password == user.encrypt(where.password):
                token = await generateUserToken(request, user.id, user.to_dict(), 7200)
                return JSON_util.response(Result.success(data=token, message="登录成功"))
            else:
                return JSON_util.response(Result.fail(message="密码不正确!"))
    ...
    async with request.ctx.session as session:
        ...
        select = (
            Select(UserPO.id,  UserPO.state, UserPO.createTime, UserPO.userName, UserPO.userGroupId,
                   UserGroupPO.name.label("groupName"), UserGroupPO.code.label(
                       "groupCode"), WxUserPO.nickName, WxUserPO.ownedAppid
                   )
...
async def add(request: Request):
    ...
    async with request.ctx.session as session:
        ...
        if isExist:
            return JSON_util.response(Result.fail(message=f"增加用户'{user.userName}'已存在"))
        user.id = None
...
async def changePwd(request: Request):
    ...
    remark = data["remark"]
    if userName == None:
        return JSON_util.response(Result.fail(message="未找到当前登录用户"))
    async with request.ctx.session as session:
        session: AsyncSession = session
        operation = DbOperations(session)
        a1: UserPO = await operation.get_one(UserPO, UserPO.userName == userName)
        if a1.password != a1.encrypt(oldPassword):
            JSON_util.response(Result.fail(message="密码不正确！"))
