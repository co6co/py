 ...
 * 关联模型
 */
export type AssociatedSelect = ITreeSelect & { associatedValue: number | boolean }
export type get_svc = (associatedId: number, data: any) => Promise<IResponse<AssociatedSelect[]>>
export type save_svc = (associatedId: number, data: IAssociation) => Promise<IResponse>
...
export default defineComponent({
  ...
  setup(prop, ctx) {
        ...
        case FormOperation.edit:
          ...
          get_association_svc(associatedId, {}).then((res) => {
            ...
            traverseTreeData(res.data, (d) => {
              if (DATA.filter) DATA.allIds.push(...[d].filter(DATA.filter).map((m) => m.id))
              ...
              if (dt.associatedValue) {
                if (DATA.filter)
                  DATA.treeDefaultChecked.push(...[d].filter(DATA.filter).map((m) => m.id))
    ...
    const rules_b: FormRules = {
      treeData: [{ required: true, validator: onValidator, trigger: ['blur', 'change'] }]
    }
    const saveBefore = () => {
    ...
    const rightCardSlots = {
      header: () => {
        return <ElFormItem label="请选择相关功能" labelWidth={135} prop="treeData"></ElFormItem>
      }
    }
    ...
    const isAllChecked = () => {
      ...
      if (nodes) {
        ...
        if (DATA.filter) checkedIds = nodes.filter((m) => DATA.filter!(m)).map((m) => m.id)
