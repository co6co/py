    ...
    <el-row>
      ...
      <el-col :span="24">
        ...
        <el-button
          ...
          type="primary"
          style="max-width: 0 0 10px 0"
          @click="isReaudit = !isReaudit"
        >
          {{ isReaudit ? '返回' : '重审' }}</el-button
        >
        <el-button
        ...
        <el-descriptions>
          <el-descriptions-item label="人工审核">
            <el-tag size="small" :type="metaData.statue2TagType(options?.manualAuditResult)"
          ...
          <el-descriptions-item label="程序审核">
            <el-tag size="small" :type="metaData.statue2TagType(options?.programAuditResult)"
              >{{ options?.programAuditResult }}-{{
                metaData.getManualStateName(
                  options?.programAuditResult == undefined ? -1 : options.programAuditResult
                )?.label
              }}</el-tag
            >
          </el-descriptions-item>
...
import { routeHook, request_resource_svc } from 'co6co-right'
...
let allowAudit = computed(() => {
  if (props.options) {
    return (
      (props.options.flowStatus in props.metaData.allowAuditStatus &&
        props.options.manualAuditResult == null) ||
      (props.options.flowStatus in props.metaData.allowAuditStatus && isReaudit.value)
    )
  }
  return false
})
...
const onAuditSumit = (statue: number) => {
  if (allowAudit.value) {
    ...
    audit_svc({ id: id, statue: statue }).then((res) => {
      if (res.code == 0) {
        ElMessage.success(res.message), emit('refesh')
      } else {
...
let info = ref<{ auditUser: { isCurrentUser: boolean; isAdminstrator: boolean } }>({
  auditUser: { isCurrentUser: false, isAdminstrator: false }
})
watch(
  () => [props.options?.id, props.options?.manualAuditResult],
  async ([neId, nestatue], [oldId, oldStatue]) => {
    if (neId) {
      info.value = (await one_svc(neId)).data
      allowReauditBtnStatus.value =
        nestatue != null &&
        (info.value.auditUser.isCurrentUser || info.value.auditUser.isAdminstrator)
    }
  }
)
...
const onDownload = () => {
  if (props.options) {
    ...
    download_one_svc(props.options.id, props.options, () => {
