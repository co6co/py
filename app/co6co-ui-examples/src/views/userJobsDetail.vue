        ...
        <div class="header">
          <el-row>
            <el-col :span="12" style="line-height: 32px">
              ...
              <el-text class="mx-1"
                >当前记录{{ currentNum }}/{{ table_module.recordTotal }}
              </el-text>
              <el-text>
                <el-tag :type="tagType.type">{{ tagType.desc }}</el-tag>
              </el-text>
              <el-text
                >审核完成：<el-tag :type="auditFinshed ? 'success' : 'danger'">{{
                  auditFinshed
                }}</el-tag>
              </el-text>
            </el-col>
            <el-col :span="12" style="line-height: 32px; text-align: right">
              <el-text class="mx-1" style="width: 216px"
                >人工审核时间:{{ table_module.currentItem?.manual_audit_time }}
              </el-text>
              <el-text class="mx-1"
                >审核用户: {{ getUserName(table_module.currentItem?.auditor_id) }}
              </el-text>
            </el-col>
          </el-row>
        </div>
