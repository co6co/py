...
import modifyDiaglog, { type Item } from '../components/sys/modifyRole'
import roleAssMenuDiaglog from '../components/sys/associated'
import {
  showLoading,
  closeLoading,
  FormOperation,
...
const onOpenDialog = (operation: FormOperation, row?: Item) => {
  table_module.diaglogTitle = operation == FormOperation.add ? '增加角色' : `编辑[${row?.name}]角色`
