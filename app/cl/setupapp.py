...
if __name__ =='__main__':
    ...
    parser.add_argument('-d','--delete',type=bool, action=argparse.BooleanOptionalAction,help="删除临时目录和文件")
    ...
    # 删除***.egg-info临时文件夹
    if args.delete: 
        gen=find_files(workDir,lambda f: "egg-info" in f ,None, '.git','node_modules','src') 
        for root,dirs, files in gen:
            for d in dirs:
    ...
    if args.target:
        extension=".tar.gz"
        gen=find_files("E:/Tools/python/py",None, lambda f:f.endswith(extension),'.git','node_modules','src') 
