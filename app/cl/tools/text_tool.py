...
def check_network_by_ping(host="8.8.8.8", count=2, timeout=2):
    """
    通过ping命令检测网络
    参数：
        host: 要ping的主机
    ...
    with open(args.source, 'r', encoding=args.encoding) as f:
        ...
        for line in lines:
            if not line or line.strip() == "":
                continue
            ipMaps[line.strip()] = None
    for ip in ipMaps:
        ipMaps[ip] = check_network_by_ping(ip)
    with open(args.output, 'w', encoding=args.output_encoding) as f:
        for ip in ipMaps:
            f.write(f"{ip} {ipMaps[ip]}\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', "--type", type=int,   default="1", help="1:JK Switch ACL,2:networkcheck")
