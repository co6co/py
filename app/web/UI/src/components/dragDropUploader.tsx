...
import { tableScope } from 'co6co-right'
import {
  ElButton,
  ElMessageBox,
  ElCard,
...
export default defineComponent({
  ...
  setup(prop, ctx) {
    ...
    const getUploadedChunks = async (subPath: string, fileName: string, totalChunks: number) => {
      ...
      buttons: () => (
        <>
          ...
          <ElButton
            ...
            onClick={onUpload}
            v-slots={{ default: () => (DATA.canelFlag ? '继续上传' : '开始上传') }}
          />
        </>
      ),
      default: () => (
                ...
                <>
                  <ElRow>
                    <ElCol span={6} class="tl">
                      <ElButton type="warning" v-slots={{ default: () => '上传文件-暂时未实现' }} />
                    </ElCol>
              ...
              {DATA.files.length == 0 ? (
                ...
                <>
                  <ElTable data={DATA.files} ref={tableRef} border={true}>
                    ...
                    <ElTableColumn label="名称" align="center" showOverflowTooltip={true}>
                      {{
                        default: (scope: { row: IFileOption }) => scope.row.file.name
                      }}
                    </ElTableColumn>
                    <ElTableColumn
                      label="文件大小"
