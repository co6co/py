...
from services.aop .wxMessageAop import MessageAOP
...
class WxView(wx_base_view):
    ...
    def reply_message(reply, crypto: WeChatCrypto, timestamp: str, nonce: str, status: int = 200):
        ...
        if crypto != None:
            ...
            return raw(reply.render(), status, headers=header)
    ...
    async def post(self, request: Request, appid: str):
        ...
        try:
            ...
            try:
                msg = WxView.getMessage(
                    request.body, crypto, msg_signature, timestamp, nonce)
