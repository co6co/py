		...
		<div class="container">
			...
			<el-table highlight-current-row @sort-change="onColChange"
			 :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header"> 
				...
				<el-table-column prop="alias" label="别名" sortable  :show-overflow-tooltip="true"></el-table-column>
				<el-table-column prop="order" label="排序" sortable  :show-overflow-tooltip="true"></el-table-column>
		...
		<el-dialog :title="form.title" v-model="form.dialogVisible" width="30%">
			<el-form label-width="70px" ref="dialogForm" :rules="rules" :model="form.fromData"> 
				<el-form-item label="名称"  prop="name">
					<el-input v-model="form.fromData.name" placeholder="英文字符"></el-input>
				</el-form-item>
				<el-form-item label="别名" prop="alias" >
					<el-input v-model="form.fromData.alias" placeholder="中文字符"></el-input>
				</el-form-item> 
				<el-form-item label="排序" prop="order" >
					<el-input-number v-model="form.fromData.order" placeholder="排序"></el-input-number>
				</el-form-item> 
			</el-form>
			<template #footer>
...
const rules: FormRules ={
	name: [ { required: true, pattern:"[0-9A-Za-z_+=-]{1,}", validator:valid_name,  message: '请输入名称', trigger: ['blur'] } ],
		...
		if(value){
			if(form.operation==0){
				add_svc(form.fromData).then(res=>{
					...
					else {ElMessage.error(`增加标签失败:${res.message}`);}
				}) 
			}else{
				edit_svc(form.id,form.fromData).then(res=>{
					...
					else {ElMessage.error(`编辑标签失败:${res.message}`);}
				})  
			} 
		}else{
