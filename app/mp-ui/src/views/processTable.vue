	...
	<div>
		<div class="container"> 
				...
				<el-row :gutter="24" v-if="table_module.queryMoreOption">
					<div class="handle-box">   
						<!--<el-switch class="el-select mr10" 
				...
				<el-row :gutter="24" @keydown.ctrl="keyDown" @keydown.prevent="keyDown"> 
					<el-col :span="14"> 
						<el-table  highlight-current-row @sort-change="onColChange"  @row-contextmenu="onRowContext"
						...
						:data="table_module.tableData" border class="table" ref="tableInstance" @row-click="onTableSelect" header-cell-class-name="table-header">  
							...
							<el-table-column width="160" prop="devRecordTime" label="设备时间" sortable :sort-method="dateOrderBy" :show-overflow-tooltip="true"></el-table-column> 
							<el-table-column width="160" prop="webRecordTime" label="平台时间" sortable :sort-method="dateOrderBy"  :show-overflow-tooltip="true"></el-table-column> 
							<el-table-column width="160" prop="auditDownloadTime" label="审核下载" sortable :sort-method="dateOrderBy" :show-overflow-tooltip="true"></el-table-column> 
							<el-table-column width="160" prop="auditInferTime" label="审核推理" sortable :sort-method="dateOrderBy" :show-overflow-tooltip="true"></el-table-column> 
							<el-table-column width="160" prop="auditAutoPushTime" label="自动推送" sortable :sort-method="dateOrderBy" :show-overflow-tooltip="true"></el-table-column> 
							<el-table-column width="160" prop="manualAuditTime" label="人工审核" sortable :sort-method="dateOrderBy" :show-overflow-tooltip="true"></el-table-column> 
							<el-table-column width="160" prop="manualPushTime" label="人工下发" sortable :sort-method=" dateOrderBy" :show-overflow-tooltip="true"></el-table-column> 
						</el-table>
					...
					<el-col :span="10">  
						<process-data-view ref="process_view" :options="table_module.currentItem" :meta-data="form_attach_data" @refesh="onRefesh" ></process-data-view>
					</el-col>
				</el-row>


		</div>  
		<div class="menuInfo" v-if="table_menuInfo.visible" :style="{ left: table_menuInfo.left + 'px', top: table_menuInfo.top + 'px' }" style="position:fixed; z-index: 9;">
			<el-menu   
				mode="vertical"
				@select="onSelectMenu"
			>
				<el-menu-item index="1">查看日志</el-menu-item>
			</el-menu>
		</div> 
		<!-- 弹出框 -->
		<el-dialog title="日志信息"  v-model="showLog" width="90%" style="height:90%;overflow: hidden;;" > 
			<log :title="LogData.title" :content="LogData.content"></log>
			<template #footer>
...
import { queryList_svc ,position_svc,start_download_task ,get_log_content_svc } from '../api/process';
import  { processDataView ,tree,selectTree,types} from '../components/process';   
...
const onSelectMenu=async ()=>{
	...
	if(id) { 
		if (table_menuInfo.row.auditLogPath)
			...
			get_log_content_svc(id).then(res=>{LogData.value.content=res.data}).catch(err=>(LogData.value.content=err.response.statusText,console.error(err)))
