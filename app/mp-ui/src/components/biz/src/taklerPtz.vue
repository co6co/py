...
<script setup lang="ts">
	...
	import { query_svc} from '../../../api/topic';
  
	const props = defineProps({
		data: {
			type: Object as PropType<dType.CurrentItem>,
		},
	});
	const talkerRef = ref(); 
	//对讲日志打印
	...
	const defaultTopic="/MANSCDP_cmd"
	const topicRef=ref(defaultTopic)
	//绑定数据发生变化
	watch(
		() => props.data,
		(n, o) => {
			...
			if (props.data){ 
				if(props.data.box){
					query_svc("ptz",props.data.box.id ).then((res)=>{
						topicRef.value=res.data.topic;
						console.info(topicRef.value)
					})
				} else{
					console.warn("主题未配置使用默认主题")
					topicRef.value=defaultTopic
				}
	...
	const onDeviceOnline=(data:gbTaklerOnlineList)=>{
		let f=false
		if(props.data&&props.data.box&&props.data.box.talkbackNo>-1&&data&&data.count>0){ 
			for(let i=0;i<data.data.length;i++){
				let item=data.data[i];
				if(item['device-id']==props.data.box.talkbackNo){
	...
	const onDeviceSession=(data:gbTaklerOnlineSessionList)=>{  
		if(props.data&&props.data.box&&props.data.box.talkbackNo>-1&&data){
			let f=false
			for(let i=0;i<data.data.length;i++){
				let item=data.data[i];
				if(item['call-info'].device==props.data.box.talkbackNo){
	...
	const ptzEnable = computed(() => {
		if (props.data?.device &&props.data?.device.sip) {
			return true;
		}
		return false;
	});
	const talkerEnable = computed(() => {
		if (props.data?.box &&props.data.box.talkbackNo) {
			return true;
		}
		return false;
	});
	...
	const checkMessageQueueState = () => {
		messangeSendRef.value.send = Ref_Mqtt.value?.client.queue.length == 0;
		if (Ref_Mqtt.value) messangeSendRef.value.length = Ref_Mqtt.value?.client.queue.length;
	};
