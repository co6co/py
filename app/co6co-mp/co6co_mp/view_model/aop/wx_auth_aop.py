...
class Authon_param:
    ...
    def setState(self, state: str):
        ...
        if len(arr) == 4:
            # arr[0] #随机码
            self.scope = wx_authon_scope.snsapi_base if arr[
                1] == wx_authon_scope.snsapi_base.val else wx_authon_scope.snsapi_userinfo
...
def oauth(method):
    ...
    async def warpper(request: Request, param: Authon_param):
        try:
            ...
            oauth = WeChatOAuth(param.appid, config.appSecret,
                                param.url, scope=param.scope.key, state=param.state)
