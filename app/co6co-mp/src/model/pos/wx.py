...
class WxMenuPO(UserTimeStampedModelPO):
    ...
    openId = Column("open_id", String(64), unique=True, comment="公众号ID")
...
class WxUserPO(TimeStampedModelPO):
    ...
    id = Column("id", Integer, comment="主键",
                autoincrement=True, primary_key=True)
    accountUid = Column("account_uid", ForeignKey(
        f"sys_account.uuid", ondelete="CASCADE"), nullable=False, index=True)
    openid = Column("open_id", String(64),  comment="用户OpenID", nullable=False)
    nickName = Column("nick_name", String(255).with_variant(
        VARCHAR(255, charset="utf8mb4"), "mysql", "mariadb"), comment="nick_name")
